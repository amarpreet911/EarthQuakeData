...
/**
 * Created by amarpreet911 on 9/27/17.
 */
...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            JSONObject readJsonString = new JSONObject(SAMPLE_JSON_RESPONSE);

            JSONArray features = readJsonString.getJSONArray("features");

            for(int i = 0; i < features.length(); i++){
                JSONObject specificFeature = features.getJSONObject(i);

                JSONObject properties = specificFeature.getJSONObject("properties");
                Earthquake earthquakeObj = new Earthquake(properties.getString("mag"), properties.getString("place"),
